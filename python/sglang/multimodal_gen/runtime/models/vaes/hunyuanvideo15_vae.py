...
# https://github.com/huggingface/diffusers/blob/main/src/diffusers/models/autoencoders/autoencoder_kl_hunyuanvideo15.py

# Copyright 2025 The Hunyuan Team, The HuggingFace Team and The sglang-diffusion Team. All rights reserved.
#
...
class HunyuanVideo15RMSNorm(nn.Module):
    """RMS norm over the channel dim; parameter name `gamma` matches the checkpoint."""
...
class HunyuanVideo15Upsample(nn.Module):
    ...
    @staticmethod
    def _dcae_upsample_rearrange(
        tensor: torch.Tensor, r1: int = 1, r2: int = 2, r3: int = 2
    ) -> torch.Tensor:
...
class HunyuanVideo15Downsample(nn.Module):
    ...
    @staticmethod
    def _dcae_downsample_rearrange(
        tensor: torch.Tensor, r1: int = 1, r2: int = 2, r3: int = 2
    ) -> torch.Tensor:
...
class HunyuanVideo15ResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        # diffusers uses get_activation("swish"), which is nn.SiLU
...
class HunyuanVideo15MidBlock(nn.Module):
    ...
    def __init__(
        ...
        resnets = [
            HunyuanVideo15ResnetBlock(in_channels=in_channels, out_channels=in_channels)
        ]
        attentions: list[HunyuanVideo15AttnBlock | None] = []
...
class HunyuanVideo15Encoder3D(nn.Module):
    r"""
    Causal encoder for 3D video-like data used in HunyuanVideo-1.5.
    """
    ...
    def __init__(
        ...
        self.mid_block: HunyuanVideo15MidBlock | None = None
...
class HunyuanVideo15Decoder3D(nn.Module):
    r"""
    Causal decoder for 3D video-like data used in HunyuanVideo-1.5.
    """
...
class AutoencoderKLHunyuanVideo15(ParallelTiledVAE):
    r"""
    ...
    def __init__(self, config: HunyuanVideo15VAEConfig) -> None:
