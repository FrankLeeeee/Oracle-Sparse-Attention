...
"""Reference estimator and block selection for X-Attention.

VERBATIM COPY from https://github.com/mit-han-lab/x-attention @ e379887
(2025-07-06): ``xattn/src/utils.py`` lines 44-191 (``find_blocks_chunked``)
and ``xattn/src/Xattention.py`` lines 13-299 (``xattn_estimate``). Only the
module-level imports are replaced — upstream imports Triton kernels and
``block_sparse_attn``, neither installed here — so every function body below is
byte-for-byte upstream, and a test against it is a test against upstream.

``xattn_estimate`` disables its own Triton path by name-checking the device
(``"100" not in device_name``), which is false for an H200, so the pure-PyTorch
branch runs. The stubs below turn any accidental Triton call into a loud failure
...
def _triton_kernel_unavailable(*args, **kwargs):  # pragma: no cover
    raise AssertionError("upstream Triton path taken; the reference must not use it")


flat_group_gemm = _triton_kernel_unavailable
softmax_fuse_block_sum = _triton_kernel_unavailable
flat_group_gemm_fuse_reshape = _triton_kernel_unavailable
block_sparse_attn_func = _triton_kernel_unavailable
