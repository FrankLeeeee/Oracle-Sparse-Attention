...
**Status: single-chunk control path works end-to-end and demonstrably steers.**
Conversion, loading, control fusion and controllability are all verified on GPU
against a real driving video. What remains is production-grade extractors and
the long-video machinery (chunk loop, history, global normalization). See the
phase table and "Next step" below.
...
| 2 — global normalization, unified noise, chunk loop + history | not started |
...
1. global control normalization, unified noise init, chunked loop with history —
   the actual "ultra-long" part; the DiT's token-count check becomes `>` here
2. USP/SP (currently an explicit `NotImplementedError` — the control streams are
   unsharded, so the tail-slice fusion would steer the wrong tokens off-rank)
3. `gpu_cases` entry, unit tests for the normalization and chunk math

## Limitations / risks

- Only the **single-chunk** path is exercised. History conditioning, the chunk
  loop and global normalization are untouched, so long-video quality is unknown.
