...
MODEL_NOTES: dict[str, dict] = {
    "self_forcing": {
        ...
        "quality_bullets": [
            ...
            "本轮 OSA 采用精度专项采纳的 <code>num_recent_frames=2</code>"
            "（近两帧整帧稠密），几何密度下限升至 ~0.29，因此 0.2 档不再存在；"
            "0.3 档实际密度 0.293，与其他方法同档可比。",
        ...
        "prompt_bullets": [
            "<b>密度与画面内容无关</b>：OSA（0.293）、LightForcing（0.300）、"
    ...
    "causal_forcing": {
        ...
        "walltime_bullets": [
            ...
            "Rolling Forcing 之间：<b>OSA 最快</b>（0.31 档 22.5 s，1.89×），"
        ...
        "prompt_bullets": [
            "<b>密度与画面内容无关</b>：OSA（0.306）、Radial（0.295）、STA（0.300）"
    ...
    "rolling_forcing": {
        ...
        "walltime_bullets": [
            "<b>封顶窗口把“整帧保留型”方法的密度下限抬高了</b>：OSA 下限 0.385、"
            "Radial 0.443、SVG2 0.619——它们的固定保留（自身 chunk + sink + 近期帧）"
            "在 21 帧的可见集合里就已占掉大半，于是最高只能到 1.49× / 1.29× / 0.83×。",
        ...
        "quality_bullets": [
            ...
            "<b>OSA（0.39）保持人物与街道结构</b>，后段店面内容有替换，属内容漂移。",
        ...
        "prompt_bullets": [
            "<b>密度完全与内容无关</b>：OSA（0.385）、LightForcing（0.286）、"
